...
FLocalSimulation::FLocalSimulation()
{
...
void FLocalSimulation::SetNumActiveBodies(uint32 InNumActiveBodies)
{
...
#if WITH_PHYSX
FJointHandle* FLocalSimulation::CreateJoint(PxD6Joint* Joint, FActorHandle* Actor1, FActorHandle* Actor2)
{
...
void FLocalSimulation::SetIgnoreCollisionPairTable(const TArray<FIgnorePair>& InIgnoreTable)
{
...
void FLocalSimulation::SetIgnoreCollisionActors(const TArray<FActorHandle*>& InIgnoreCollisionActors)
{
...
void FLocalSimulation::RemoveActor(FActorHandle* Handle)
{
	if(Handle != nullptr)
	{
...
void FLocalSimulation::RemoveJoint(FJointHandle* Handle)
{
	if (Handle != nullptr)
	{
		// store ref to index, swap with last (should move to apporpiate position to maintain sorting)
		auto index = Handle->JointDataIndex;

		// will be done to maintain sorting todo: maintaining sorting
		if (JointHandles.Num() > 1)
		{
			if (JointHandles[JointHandles.Num() - 1] != nullptr)
			{
				// this allows us to remove without unsorting
				SwapJointData(JointHandles.Num() - 1, index);

				// store new index
				index = JointHandles.Num() - 1;
			}
		}

		// store temp before removal from array container
		auto temp = JointHandles[Handle->JointDataIndex];

		// remove data associated with handle
		JointHandles.Remove(Handle);
		JointData.RemoveAt(index);
...
FActorHandle* FLocalSimulation::CreateDynamicActor(PxRigidDynamic* RigidDynamic, const FTransform& TM)
{
...
FActorHandle* FLocalSimulation::CreateKinematicActor(PxRigidBody* RigidBody, const FTransform& TM)
{
	const uint32 ActorDataIndex = CreateActor<ECreateActorType::KinematicActor>(RigidBody, TM);
	return ActorHandles[ActorDataIndex];
}

FActorHandle* FLocalSimulation::CreateStaticActor(PxRigidActor* RigidActor, const FTransform& TM)
{
...
void FLocalSimulation::SwapActorData(uint32 Actor1DataIdx, uint32 Actor2DataIdx)
{
...
void FLocalSimulation::SwapJointData(uint32 Joint1Idx, uint32 Joint2Idx)
{
...
void FLocalSimulation::ValidateArrays() const
{
...
void FLocalSimulation::EvictCache()
{
...
DECLARE_DWORD_COUNTER_STAT(TEXT("Simulated Bodies"), STAT_NumSimulatedBodies, STATGROUP_LocalPhysics);
DECLARE_DWORD_COUNTER_STAT(TEXT("Active Simulated Bodies"), STAT_NumActiveSimulatedBodies, STATGROUP_LocalPhysics);
DECLARE_DWORD_COUNTER_STAT(TEXT("Kinematic Bodies"), STAT_NumKinematicBodies, STATGROUP_LocalPhysics);

DECLARE_CYCLE_STAT(TEXT("FLocalSimulation::Simulate"), STAT_LocalSimulate, STATGROUP_LocalPhysics);

void FLocalSimulation::Simulate(float DeltaTime, const FVector& Gravity)
{
	...
	if(DeltaTime > 0)
	{
		SCOPE_CYCLE_COUNTER(STAT_LocalSimulate);
...
DECLARE_CYCLE_STAT(TEXT("ConstructSolverBodies"), STAT_LocalConstructSolverBodies, STATGROUP_LocalPhysics);

void FLocalSimulation::ConstructSolverBodies(float DeltaTime, const FVector& Gravity)
{
#if WITH_PHYSX
	SCOPE_CYCLE_COUNTER(STAT_LocalConstructSolverBodies);
...
DECLARE_CYCLE_STAT(TEXT("PrepareIterationCache"), STAT_LocalPrepareIterationCache, STATGROUP_LocalPhysics);

void FLocalSimulation::PrepareIterationCache()
{
#if WITH_PHYSX
	SCOPE_CYCLE_COUNTER(STAT_LocalPrepareIterationCache);
...
DECLARE_CYCLE_STAT(TEXT("GenerateContacts"), STAT_LocalGenerateContacts, STATGROUP_LocalPhysics);

void FLocalSimulation::GenerateContacts()
{
#if WITH_PHYSX
	SCOPE_CYCLE_COUNTER(STAT_LocalGenerateContacts);
...
bool GBatchJoints = false;	//TODO: temp hack to fix crash in Local mode due to joint batching

DECLARE_CYCLE_STAT(TEXT("BatchConstraints"), STAT_LocalBatchConstraints, STATGROUP_LocalPhysics);

void FLocalSimulation::BatchConstraints()
{
#if WITH_PHYSX
	SCOPE_CYCLE_COUNTER(STAT_LocalBatchConstraints);
...
DECLARE_CYCLE_STAT(TEXT("PrepareConstraints"), STAT_LocalPrepareConstraints, STATGROUP_LocalPhysics);

void FLocalSimulation::PrepareConstraints(float DeltaTime)
{
#if WITH_PHYSX
	SCOPE_CYCLE_COUNTER(STAT_LocalPrepareConstraints);
...
DECLARE_CYCLE_STAT(TEXT("SolveAndIntegrate"), STAT_LocalSolveAndIntegrate, STATGROUP_LocalPhysics);


void FLocalSimulation::SolveAndIntegrate(float DeltaTime)
{
#if WITH_PHYSX
	SCOPE_CYCLE_COUNTER(STAT_LocalSolveAndIntegrate);
...
void FLocalSimulation::AddRadialForce(int32 ActorDataIndex, const FVector& InOrigin, float Strength, float Radius, ERadialImpulseFalloff Falloff, EForceType ForceType)
{
#if WITH_PHYSX
